...
void test21_and_success(t_shell *shell)
{
	...
	t_ast *cmd = ast_and(
		ast_cmd((char *[]){"/usr/bin/echo", "hello", NULL}),
		ast_cmd((char *[]){"/usr/bin/echo", "world", NULL})
	);
	int ret = ft_exec_root(cmd, shell);
...
void test22_and_fail(t_shell *shell)
{
	...
	t_ast *cmd = ast_and(
		...
		ast_cmd((char *[]){"/usr/bin/echo", "wontexecute", NULL})
	);
	int ret = ft_exec_root(cmd, shell);
...
void test23_or_success(t_shell *shell)
{
	...
	t_ast *cmd = ast_or(
		...
		ast_cmd((char *[]){"/usr/bin/echo", "notexecuted", NULL})
	);
	int ret = ft_exec_root(cmd, shell);
...
void test24_or_fail(t_shell *shell)
{
	...
	t_ast *cmd = ast_or(
		ast_cmd((char *[]){"/bin/false", NULL}),
		ast_cmd((char *[]){"/usr/bin/echo", "executed", NULL})
	);
	int ret = ft_exec_root(cmd, shell);
...
void test25_and_pipe(t_shell *shell)
{
	...
	t_ast *cmd = ast_and(
		...
		ast_cmd((char *[]){"/usr/bin/echo", "b", NULL})
	);
	int ret = ft_exec_root(cmd, shell);
...
void test27_and_subshell(t_shell *shell)
{
	// (echo hi) && (echo bye)
	t_ast *cmd = ast_and(
		ast_subshell(ast_cmd((char *[]){"/usr/bin/echo", "hi", NULL})),
		ast_subshell(ast_cmd((char *[]){"/usr/bin/echo", "bye", NULL}))
	);
	int ret = ft_exec_root(cmd, shell);
...
void test28_or_subshell_fail(t_shell *shell)
{
	...
	t_ast *cmd = ast_or(
		...
		ast_subshell(ast_cmd((char *[]){"/usr/bin/echo", "executed", NULL}))
	);
	int ret = ft_exec_root(cmd, shell);
...
void test30_and_or_subshell_redir(t_shell *shell)
{
	...
	t_ast *cmd = ast_or(
		ast_and(
			ast_redir_out(ast_cmd((char *[]){"/usr/bin/cat", NULL}), "file1"),
			ast_redir_out(ast_cmd((char *[]){"/usr/bin/echo", "b", NULL}), "file2")
		),
		ast_redir_out(ast_cmd((char *[]){"/usr/bin/echo", "c", NULL}), "file3")
	);
	int ret = ft_exec_root(cmd, shell);
